		...
		if (s == ENTER)
		{
			...
			if(checkSVDaThi(k->info) == false)
			{
				if(ExitXoa("Ban co chac chan muon xoa sinh vien nay?(Y/N)", s, sCheck) == true)
				{
						...
						XoaTB(130,26);
						gotoxy(132,30);
						string tb = "Xoa sinh vien thanh cong";
						InTB(tb,129,30);
						AnConTro();
						Sleep(1000);
						XoaTB(130,26);
			...
			else
			{
				...
				XoaTB(130,26);
				gotoxy(129,30);
				string tb = "Sinh vien da tham gia thi! Khong the xoa!";
				InTB(tb,129,30);
				Sleep(1000);
				XoaTB(130,26);
				AnConTro();
...
void FunctionCauHoi(DSMH & dsm, dslop dsl, int id, int idlonnhat)
{
	...
	while(1)
	{
		...
		if(s == DOWN && sCheck == false)
		{
			if(state == 9)
			{
				if(pTam->mh.cauHoiMH.listCauHoi[state + page*10 + 1].noiDung.compare("") != 0 && (page*10 +state + 1) < pTam->mh.cauHoiMH.n)
				{
					gotoxy(wherex()-1,wherey());
						TextColor(9);
					cout << " ";
	...
	while(1)
	{
		...
		if ( s == ENTER)
		{
			...
			if(state == 5)
			{
				...
				||  ch.B.compare(ch.C)  == 0||  ch.B.compare(ch.D)  == 0||  ch.C.compare(ch.D) == 0)
				{
					string tb = "Cac dap an khong duoc trung nhau!";
					InTB(tb,129,30);
					AnConTro();
					Sleep(1000);
					XoaTB(130,26);
...
{	
	...
	if(slCH != 0){
		...
		XoaTB(130,26);
		gotoxy(129,30);
		string tb = "Mon hoc da co cau hoi!        Khong the sua!";
		InTB(tb,129,30);
		AnConTro();
		Sleep(1000);
		XoaTB(130,26);
...
void SuaLop(dslop dsl, lop *&l, int vitri)
{
	...
	while(1)
	{
		...
		if(s == ENTER)
		{
			if(tenlop.length() == 0 || checkChuoiRong(tenlop) == false)
			{
				string tb = "Khong de trong ten lop!";
				gotoxy(129,30);
				InTB(tb,129,30);
				AnConTro();
				Sleep(1000);
...
// ==================== FUNCTION LUA CHON GV THI ==========================
void FunctionLuaChonGV(dslop &dsl, DSMH &dsm, dssv &ds, int idlonnhat, int arr1[], int arr2[])
{
	system("cls");
	gotoxy(0,0);
	veKhungThuCong();
...
int countDemThiMotMonTrongSV(dsdiemThi dsDiem, int idMon){
	int dem = 0;
	for(nodediemThi *p = dsDiem.First; p != NULL; p = p->pNext){
		if(p->info.idMonHoc == idMon){
			dem++;
				...
				if(StringToInt(ntg) > 0 && StringToInt(nsct) > 0)
					{
						...
						if (sct > sch)
						{
							string tb = "Ngan hang de thi hien co " + IntToString(sch) + " cau hoi. Vui long chon so cau thi hop le!";
							InTB(tb,129,30);
							AnConTro();
							Sleep(2000);
							XoaTB(130,26);
...
void XemBangDiemSV(nodeSV *k,int idMon, DSCH dsch)
{
	system("cls");
	gotoxy(0,0);
	veKhungThuCong();
	...
	int maxpage;
	int page = 0;
	int sl = countDemThiMotMonTrongSV(k->info.diem, idMon);
//	gotoxy(0,0);
//	cout<<"ID MON: "<<idMon<<"       So lan thi: "<<sl<<endl; 
//	system("pause");
	int *arr [sl];
	for(int i = 0; i < sl; i++)
	{
	...
	nodediemThi *dt = k->info.diem.First;
	int dem = 0;
	int vitriArr = 0;
	while(dt != NULL)
	{
		if(idMon == dt->info.idMonHoc)
		{
			*arr[vitriArr] = dem;
			vitriArr++;
		}
	...
	while(s != ESC)
	{
		...
		if (s == ENTER)
		{
			...
			for (int i = 0; i < *arr[state + page*10]; i++)
			{
				if (p->pNext != NULL)
				{
					p = p->pNext;
	...
	while(s != ESC)
	{
		...
		if(s == ENTER)
		{
			...
			else
			{
				...
				{
					XemBangDiemSV(k, monhoc->id ,monhoc->mh.cauHoiMH);
					system("cls");
					gotoxy(0,0);
					veKhungThuCong();
					gotoxy(10,1);
...
void mainProcess(dslop &dsl, dssv &ds, DSMH &dsm, int arr1[], int arr2[])
{
	...
	while(1)
	{
		...
		case 1:
			{
				system("cls");
				FunctionLuaChonGV(dsl, dsm, ds, idlonnhat, arr1, arr2);
				break;
			}	
		case -1:
			{
